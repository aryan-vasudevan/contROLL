...
bool begin() {
  ...
  // Measured on this board: the reader answers at 0x26, below the 0x28 the
  // datasheet's 0101-ADR scheme predicts, so the sweep starts at 0x20. The
  // accelerometer at 0x19 is the only other device on the bus and sits below
  // the range, so nothing else can be mistaken for the reader.
  for (uint8_t addr = 0x20; addr <= 0x2F; addr++) {
    Wire.beginTransmission(addr);
    if (Wire.endTransmission() != 0) continue;
    gAddr = addr;
    const uint8_t v = readReg(REG_VERSION);
    // 0x91 and 0x92 are NXP's documented revisions, but the clones this badge
    // was built with report other values -- 0x82 on the board this was tested
    // against. Reject only an all-zero or all-ones read, which is what a dead
    // or absent part looks like; anything else gets driven and allowed to fail
    // honestly at REQA instead.
    if (v != 0x00 && v != 0xFF) { gVersion = v; gPresent = true; break; }
    gAddr = 0;
  }
